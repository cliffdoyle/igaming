<?php
/**
 * Reusable operator-review layout (casino reviews + sportsbook reviews).
 * Renders the review box, review content, author box and sticky CTA against
 * the current main query post. Section labels/CTA verb are configurable via
 * get_template_part( ..., $args ) so the same markup works under Online Casinos
 * or under Sports Picks.
 */
if (!defined('ABSPATH')) {
    exit;
}

$og_sec_label = isset($args['sec_label']) ? $args['sec_label'] : 'Online Casinos';
$og_sec_url   = isset($args['sec_url'])   ? $args['sec_url']   : home_url('/online-casinos/');
$og_cta_verb  = isset($args['cta_verb'])  ? $args['cta_verb']  : 'Visit Casino';
    ...
    <div class="content-area">
        <main class="article-content">
            ...
            <p style="font-size:0.85rem;color:var(--og-text-light);margin-bottom:1.5rem;">
                ...
                <a href="<?php echo esc_url($og_sec_url); ?>"><?php echo esc_html($og_sec_label); ?></a> /
                <?php the_title(); ?>
            </p>
                    ...
                    <div style="flex:1;">
                        ...
                        <?php if ($affiliate) : ?>
                            <div style="margin-top:0.75rem;">
                                <a href="<?php echo esc_url($affiliate); ?>" class="btn btn-play" target="_blank" rel="<?php echo esc_attr(ontariogamers_aff_rel()); ?>" style="font-size:1rem;padding:0.6rem 1.75rem;"><?php echo esc_html($og_cta_verb); ?></a>
                            </div>
                        <?php endif; ?>
                    </div>
                </div>
